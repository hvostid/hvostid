...
Every service emits structured JSON logs to stdout (one line per
event), suitable for direct ingestion by ELK / Loki / Grafana. Each
line contains:

- `timestamp` -- ISO-8601 with nanosecond precision;
- `level` -- `DEBUG` / `INFO` / `WARN` / `ERROR`;
- `service` -- service name (`api-gateway`, `auth-service`, ...);
- `logger`, `thread`, `message` -- standard Logback fields;
- `requestId` -- value of the `X-Request-Id` header (generated by
  api-gateway and propagated to downstream services via MDC);
- `userId` -- user identifier from the `X-User-Id` header when the
  request is authenticated.

Root log level is controlled via the `LOG_LEVEL` environment variable
(default `INFO`):
...
The `local` profile switches output to a human-readable pattern for
IDE debugging:
...
Every external request is tagged with a unique `requestId` that is
reused across all downstream calls. To find every log line related to
a single request, filter on this field:

```bash
docker compose logs --no-color | jq -c 'select(.requestId == "8f3e5a2c-...")'
```

In Grafana / Kibana use the query `requestId:"8f3e5a2c-..."`. When the
client does not supply `X-Request-Id`, the gateway generates a UUID and
echoes it back in the response header -- that value can be used to
look up the request.

### What must never be logged

Passwords, JWT tokens, refresh tokens, MinIO content tokens, and any
PII must not be logged. Exception messages stay on the server via
`log.error`; only a generic `detail` is returned to the client in the
RFC 7807 `problem+json` response -- see
