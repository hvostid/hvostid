...
Скрипт очищает Docker volumes, поднимает стек с `SPRING_PROFILES_ACTIVE=demo`,
применяет repeatable-миграции Flyway и загружает sample-файлы в MinIO.
Флаг `--no-wipe` сохраняет существующие volumes.
