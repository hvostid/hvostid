...
Все сервисы пишут структурированные JSON-логи в stdout (одна строка =
один JSON-объект). Формат подходит для прямого приёма в ELK / Loki /
Grafana. Каждая строка содержит:

- `timestamp` -- ISO-8601 с наносекундной точностью;
- `level` -- `DEBUG` / `INFO` / `WARN` / `ERROR`;
- `service` -- имя сервиса (`api-gateway`, `auth-service`, ...);
- `logger`, `thread`, `message` -- стандартные поля Logback;
- `requestId` -- значение заголовка `X-Request-Id` (генерируется на
  api-gateway и проксируется в downstream через MDC);
- `userId` -- идентификатор пользователя из заголовка `X-User-Id`,
  если запрос аутентифицирован.

Уровень root-логгера задаётся переменной окружения `LOG_LEVEL`
(по умолчанию `INFO`):

```bash
LOG_LEVEL=DEBUG ./gradlew :listing-service:bootRun
```

Профиль `local` переключает вывод на человекочитаемый pattern для
отладки в IDE:

```bash
SPRING_PROFILES_ACTIVE=local ./gradlew :listing-service:bootRun
```

### Фильтрация по `requestId`

Каждый внешний запрос получает уникальный `requestId`, который
переиспользуется во всех downstream-вызовах. Чтобы найти все строки
лога, относящиеся к одному запросу, отфильтруйте по этому полю:

```bash
docker compose logs --no-color | jq -c 'select(.requestId == "8f3e5a2c-...")'
```

В Grafana / Kibana используйте запрос `requestId:"8f3e5a2c-..."`. Если
клиент не передал `X-Request-Id`, gateway сгенерирует UUID и вернёт его
тем же заголовком в ответе -- это значение можно использовать для
поиска.

### Что не должно попадать в логи

Запрещено логировать пароли, JWT-токены, refresh-токены, content-tokens
файлов в MinIO и любой PII. Текст исключений сохраняется на сервере
через `log.error`, но в клиентский ответ (RFC 7807 problem+json)
утекает только обобщённый `detail` -- см.
[`common.exception.GlobalErrorHandler`](./common/src/main/java/ru/hvostid/common/exception/GlobalErrorHandler.java).
