...
import static ru.hvostid.gateway.filter.RequestIdFilter.REQUEST_ID_HEADER;
...
class RequestIdFilterTest {
    ...
    void shouldReuseClientProvidedRequestId() throws ServletException, IOException {
        ...
        assertEquals(clientId, response.getHeader(REQUEST_ID_HEADER), "Response must echo the client-provided X-Request-Id");
    }
    ...
    void shouldPropagateRequestIdToDownstreamViaRequestWrapper() throws ServletException, IOException {
        ...
        FilterChain chain = (req, _) -> downstreamHeader.set(((jakarta.servlet.http.HttpServletRequest) req).getHeader(REQUEST_ID_HEADER));

        filter.doFilterInternal(request, response, chain);

        assertNotNull(downstreamHeader.get(), "Downstream must receive X-Request-Id via wrapped request");
        assertEquals(response.getHeader(REQUEST_ID_HEADER), downstreamHeader.get(), "Downstream and response X-Request-Id must match");
    }
    ...
    void shouldPutRequestIdIntoMdcDuringFilterExecution() throws ServletException, IOException {
        ...
        assertEquals(response.getHeader(REQUEST_ID_HEADER), mdcValue.get(), "MDC requestId must match the response header");
    }
    ...
    void shouldSetResponseHeaderBeforeChainExecution() throws ServletException, IOException {
        ...
        FilterChain chain = (_, res) -> headerDuringChain.set(((MockHttpServletResponse) res).getHeader(REQUEST_ID_HEADER));
